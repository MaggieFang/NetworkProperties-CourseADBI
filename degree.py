...
# SPARK_HOME ="/Users/xfang7/Downloads/spark-2.4.0-bin-hadoop2.7"
...
def simple(g):
	...
	flipped = g.edges.selectExpr('src as dst','dst as src')
	...
	newEdges = g.edges.unionAll(flipped).distinct().filter('src != dst')
	return GraphFrame(g.vertices, newEdges)
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	# https://docs.azuredatabricks.net/spark/latest/graph-analysis/graphframes/graph-analysis-tutorial.html
	# All that you need to do is rename your name column to id in the vertices table and
	# the start and end stations to src and dst respectively for the edges table. These are the require naming conventions for vertices and edges in GraphFrames as of the time of this writing (Feb. 2016).
	pairs = lines.map(lambda s: s.split(delim))
	edges_table = sqlContext.createDataFrame(pairs,['src','dst'])
	...
	vertice_table = edges_table.selectExpr('src as id').unionAll(edges_table.selectExpr('dst as id')).distinct()

	# Create graphframe from the vertices and edges.
	g = GraphFrame(vertice_table, edges_table)
	return g
